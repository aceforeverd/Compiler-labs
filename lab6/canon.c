...
static void trace(T_stmList list) {
    ...
    /* we want false label to follow CJUMP */
        ...
        T_stmList true = (T_stmList)S_look(block_env, s->u.CJUMP.true_l);
        T_stmList false = (T_stmList)S_look(block_env, s->u.CJUMP.false_l);
        if (false) {
