...
struct expty transVar(S_table venv, S_table tenv, A_var v, Tr_level level,
    ...
    switch (v->kind) {
        case A_simpleVar: {
            ...
            return expTy(Tr_simpleVar(entry->u.var.access, level),
                         actual_ty(entry->u.var.ty));
        }
        ...
        case A_fieldVar: {
            ...
            Ty_field_wrap field_wrap =
                FieldList_lookup(t_var.ty->u.record, v->u.field.sym);
            if (!field_wrap) {
                /*
                 * filed not found
                 */
                EM_error(v->u.field.var->pos, "field %s doesn't exist",
                         S_name(v->u.field.sym));
                return expTy(NULL, Ty_Void());
            }
            return expTy(
                Tr_fieldVar(t_var.exp, field_wrap->offset * F_wordSize),
                field_wrap->field->ty);
        }
        ...
        case A_subscriptVar: {
             ...
             */
            expty_t t_var =
                transVar(venv, tenv, v->u.subscript.var, level, label);
            ...
            if (t_exp.ty->kind != Ty_int) {
                ...
                EM_error(v->u.subscript.exp->pos,
                         "exp of subscript should be int");
            }

            return expTy(Tr_subscriptVar(t_var.exp, t_exp.exp),
                         t_var.ty->u.array);
        }
    }
}

struct expty transExp(S_table venv, S_table tenv, A_exp aExp, Tr_level level,
                      Temp_label label) {
    if (!aExp) {
        return expTy(NULL, Ty_Void());
    }
    ...
    switch (aExp->kind) {
        ...
        case A_callExp: {
            ...
            if (!f_entry || f_entry->kind != E_funEntry) {
                EM_error(aExp->pos, "undefined function %s",
                         S_name(aExp->u.call.func));
                return expTy(NULL, Ty_Int());
            }
            Tr_expList list = check_function_params(
                venv, tenv, aExp->pos, aExp->u.call.args,
                f_entry->u.fun.formals, aExp->u.call.func, level, label);
            return expTy(Tr_callExp(f_entry->u.fun.label, level,
                                    f_entry->u.fun.level, list),
                         f_entry->u.fun.result);
        }
        case A_opExp: {
            A_oper oper = aExp->u.op.oper;
            struct expty left =
                transExp(venv, tenv, aExp->u.op.left, level, label);
            struct expty right =
                transExp(venv, tenv, aExp->u.op.right, level, label);
            ...
            if (oper == A_plusOp || oper == A_minusOp || oper == A_timesOp ||
                ...
                if (oper != A_eqOp && oper != A_neqOp) {
                    if (left.ty->kind != Ty_int && left.ty->kind != Ty_string) {
                        EM_error(aExp->u.op.left->pos,
                                 "Compare unsupport type");
                    }
                }
                return expTy(Tr_opExp(oper, left.exp, right.exp), Ty_Int());
            }
        ...
        case A_seqExp: {
            ...
            Ty_ty type = Ty_Void();
            Tr_expList expList =
                transSeqExp(venv, tenv, list, level, label, &type);

            return expTy(Tr_seqExp(expList), type);
        }
        ...
        case A_assignExp: {
             ...
             */
            expty_t var_ty =
                transVar(venv, tenv, aExp->u.assign.var, level, label);
            if (var_ty.ty->kind == Ty_void) {
                return expTy(NULL, Ty_Void());
            }
            expty_t exp_ty =
                transExp(venv, tenv, aExp->u.assign.exp, level, label);
            if (aExp->u.assign.var->kind == A_simpleVar) {
                ...
                if (entry && entry->readonly && entry->kind == E_varEntry) {
                    EM_error(aExp->u.assign.var->pos,
        ...
        case A_ifExp: {
            expty_t test_ty =
                transExp(venv, tenv, aExp->u.iff.test, level, label);
            expty_t then_ty =
                transExp(venv, tenv, aExp->u.iff.then, level, label);
            expty_t else_ty =
                transExp(venv, tenv, aExp->u.iff.elsee, level, label);
            if (test_ty.ty->kind != Ty_int) {
                EM_error(aExp->u.iff.test->pos,
                         "test statement is not integer");
            }
        ...
        case A_forExp: {
             ...
             * expty_t exp1_ty = transExp(venv, tenv, aExp->u.forr.lo, level,
             label);
             * if (exp1_ty.ty->kind != Ty_int) {
             *     EM_error(aExp->u.forr.lo->pos, "for exp's range type is not
             integer");
             * }

             * expty_t exp2_ty = transExp(venv, tenv, aExp->u.forr.hi, level,
             label);
             * if (exp2_ty.ty->kind != Ty_int) {
             *     EM_error(aExp->u.forr.hi->pos, "for exp's range type is not
             integer");
             * }

             * expty_t exp3_ty = transExp(venv, tenv, aExp->u.forr.body, level,
             label);
             * if (exp3_ty.ty->kind != Ty_void) {
             *     EM_error(aExp->u.forr.body->pos, "for loop body must return
             no value");
             * }
             ...
             * // return transExp(venv, tenv, A_TryForToLetExp(aExp), level,
             label);
             * return expTy(NULL, Ty_Void());
             */
        }
        ...
        case A_arrayExp: {
            ...
            if (!base_ty) {
                EM_error(aExp->pos, "array type %s not defined",
                         S_name(aExp->u.array.typ));
                return expTy(NULL, Ty_Void());
            }
            expty_t array_ty =
                transExp(venv, tenv, aExp->u.array.init, level, label);
            expty_t size_ty =
                transExp(venv, tenv, aExp->u.array.size, level, label);
            if (array_ty.ty->kind != base_ty->u.array->kind) {
                EM_error(aExp->u.array.init->pos, "type mismatch");
            }
            if (size_ty.ty->kind != Ty_int) {
                EM_error(aExp->u.array.size->pos,
                         "array size should be integer");
            }
        ...
        case A_recordExp: {
            ...
            if (!record_ty) {
                EM_error(aExp->pos, "undefined type %s",
                         S_name(aExp->u.record.typ));
                return expTy(NULL, Ty_Record(NULL));
            }
            if (record_ty->kind != Ty_record) {
                EM_error(aExp->pos, "%s is not a record",
                         S_name(aExp->u.record.typ));
                return expTy(NULL, Ty_Name(aExp->u.record.typ,
                                           Ty_Record(record_ty->u.record)));
            }
            ...
            if (!aExp->u.record.fields && record_ty->u.record) {
                ...
                expList = check_fields_match(aExp->pos, venv, tenv,
                                             aExp->u.record.fields,
                                             record_ty->u.record, level);
            }

            /* A_exp trExp = A_fieldToExp(aExp->pos, aExp->u.record.fields,
             * aExp->u.record.typ); */
...
Tr_exp transDec(S_table venv, S_table tenv, A_dec dec, Tr_level level,
    ...
    switch (dec->kind) {
        case A_varDec: {
            ...
            if (e.ty->kind == Ty_nil) {
                if (!dec->u.var.typ || strlen(S_name(dec->u.var.typ)) == 0) {
                ...
                if (e.ty->kind != var_ty->kind) {
                    if (e.ty->kind == Ty_nil) {
                        if (var_ty->kind != Ty_record) {
                            EM_error(dec->u.var.init->pos,
                                     "Nil can't not assign to var that is not "
                                     "record");
                        }
                    } else {
...
Tr_expList check_function_params(S_table venv, S_table tenv, int pos,
                                 A_expList args, Ty_tyList standards,
                                 S_symbol func, Tr_level level,
                                 Temp_label label) {
    ...
    if (standards == NULL && args != NULL) {
        EM_error(args->head->pos, "too many params in function %s",
                 S_name(func));
        return NULL;
    }
    ...
    return Tr_ExpList(
        ty.exp, check_function_params(venv, tenv, args->head->pos, args->tail,
                                      standards->tail, func, level, label));
}
...
Tr_expList check_fields_match(int pos, S_table venv, S_table tenv,
    ...
    Tr_exp tExp =
        check_field_match(venv, tenv, args->head, standards->head, level);
    return Tr_ExpList(
        tExp, check_fields_match(args->head->exp->pos, venv, tenv, args->tail,
                                 standards->tail, level));
}

Tr_exp check_field_match(S_table venv, S_table tenv, A_efield ef, Ty_field tf,
                         Tr_level level) {
    if (ef->name != tf->name) {
...
int checkTypeDecCircles(S_table tenv, A_nametyList types) {
    ...
    ls = types;
    while (ls) {
        if (checkTypeDecCircle(tenv, ls->head, n) == 1) return 1;
...
Ty_fieldList transFieldList(S_table tenv, A_fieldList list) {
    ...
    return Ty_FieldList(Ty_Field(f->name, ty),
                        transFieldList(tenv, list->tail));
}
...
void transFunDecHeader(S_table venv, S_table tenv, A_fundecList fundecList,
    ...
    if (fun_entry != NULL && fun_entry->kind != E_funEntry) {
        EM_error(fundecList->head->pos, "two functions have the same name",
                 fundecList->head->pos);
    } else {
        Temp_label func_name = Temp_namedlabel(S_name(fundecList->head->name));
        S_enter(venv, fundecList->head->name,
                E_FunEntry(
                    Tr_newLevel(level, func_name,
                                paramsToEscapes(fundecList->head->params)),
                    func_name, transFuncParams(tenv, fundecList->head->params),
                    result_ty));
    }
...
Tr_expList transFunDecBody(S_table venv, S_table tenv, A_fundecList fundecList,
                           Tr_level level) {
    /*
     * parse function body
     */
    ...
    if (fundecList->head->result &&
        ...
        if (body_ty.ty->kind != result_ty->kind) {
            EM_error(fundecList->head->pos,
                     "function return type not match body");
        }
    } else {
        if (body_ty.ty->kind != Ty_void) {
...
Ty_tyList transFuncParams(S_table tenv, A_fieldList params) {
    ...
    return Ty_TyList(type, transFuncParams(tenv, params->tail));
}
...
Tr_expList transSeqExp(S_table venv, S_table tenv, A_expList list,
    ...
    expty_t ty = transExp(venv, tenv, list->head, level, label);
    *type = ty.ty;
    return Tr_ExpList(ty.exp,
                      transSeqExp(venv, tenv, list->tail, level, label, type));
}
